...
class RedManager {
  ...
  async init(express) {
    ...
    express.use(ifHas(process.env.LINTO_BLS_RED_UI_PATH, redSettings.httpAdminRoot), RED.httpAdmin)
    express.use(redSettings.httpNodeRoot, RED.httpNode)

    server.listen(ifHas(process.env.LINTO_BLS_RED_HTTP_PORT, redSettings.uiPort))
